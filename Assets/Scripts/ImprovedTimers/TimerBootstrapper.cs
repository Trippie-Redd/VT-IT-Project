...
namespace ImprovedTimers
{
    internal static class TimerBootstrapper
    {
        ...
        internal static void Initialize()
        {
            ...
            if (!_InsertTimerManager<Update>(ref currentPlayerLoop, 0))
            {
        ...
        private static void _OnPlayModeState(PlayModeStateChange state)
        {
            if (state == PlayModeStateChange.ExitingPlayMode)
            {
                PlayerLoopSystem currentPlayerLoop = PlayerLoop.GetCurrentPlayerLoop();
                _RemoveTimerManager<Update>(ref currentPlayerLoop);
                PlayerLoop.SetPlayerLoop(currentPlayerLoop);
        ...
        private static void _RemoveTimerManager<T>(ref PlayerLoopSystem loop)
        {
            PlayerLoopUtils.RemoveSystem<T>(ref loop, in _timerSystem);
        }

        private static bool _InsertTimerManager<T>(ref PlayerLoopSystem loop, int index)
        {
