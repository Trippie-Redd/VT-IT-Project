...
namespace UnityUtils.LowLevel
{
    public static class PlayerLoopUtils
    {
        public static void RemoveSystem<T>(ref PlayerLoopSystem loop, in PlayerLoopSystem systemToRemove)
        {
            ...
            _HandleSubSystemLoopForRemoval<T>(ref loop, systemToRemove);
        }

        private static void _HandleSubSystemLoopForRemoval<T>(ref PlayerLoopSystem loop, PlayerLoopSystem systemToRemove)
        {
            if (loop.subSystemList == null) return;
        ...
        public static bool InsertSystem<T>(ref PlayerLoopSystem loop, in PlayerLoopSystem systemToInsert, int index)
        {
            if (loop.type != typeof(T))
                return _HandleSubSystemLoop<T>(ref loop, systemToInsert, index);
        ...
        private static bool _HandleSubSystemLoop<T>(ref PlayerLoopSystem loop, in PlayerLoopSystem systemToInsert,
            int index)
        {
        ...
        public static void PrintPlayerLoop(PlayerLoopSystem loop)
        {
            ...
            foreach (PlayerLoopSystem subSystem in loop.subSystemList)
                _PrintSubSystem(subSystem, sb, 0);
            
            Debug.Log(sb.ToString());
        }

        private static void _PrintSubSystem(PlayerLoopSystem system, StringBuilder sb, int level)
        {
            ...
            foreach (PlayerLoopSystem subSystem in system.subSystemList)
                _PrintSubSystem(subSystem, sb, level + 1);
        }
    }
}